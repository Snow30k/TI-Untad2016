...
uses
  Winapi.Windows, System.SysUtils, System.Classes, mainForm;
...
//  d := 0;
//  if (Form1.Matrix <> nil) and (path <> nil) then
//    for i := 0 to n-1 do
//    begin
//      if path[i] < Length(path) then
//        d := d + Form1.Matrix[path[i]][path[i+1]]
//      else
//      begin
//        Result := d;
//        exit ;
...
//      begin
//        copyOrder(bee[i], Form1.bestSolution);
//        recordDist := nectar[i];
//        ToStr := '';
//        for a := 0 to Length(Form1.bestSolution)-1 do ToStr := ToStr + IntToStr(Form1.bestSolution[a]);
...
//  end;
//  ToStr := '';
//  for a := 0 to Length(Form1.bestSolution)-1 do ToStr := ToStr + Chr(65+Form1.bestSolution[a]);
//  Form1.Memo1.Lines.Append('');
